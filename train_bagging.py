...
train_bagging.py
----------------
Train, tune, and evaluate the scratch-built Bagging ensemble on the
Heart Failure Prediction dataset.

Usage
-----
    python train_bagging.py [--csv path/to/heart.csv]
"""
...
def main(csv_path: str | None = None) -> None:
    ...
    # ------------------------------------------------------------------
    print("\n" + "=" * 60)
    print("  HEART FAILURE PREDICTION — BAGGING ENSEMBLE")
    print("=" * 60)
    ...
    final_model = BaggingClassifier(**best_params, random_seed=RANDOM_SEED)
    ...
    summary_path = os.path.join("results", "bagging_results.txt")
    with open(summary_path, "w") as f:
        f.write("BAGGING ENSEMBLE — RESULTS SUMMARY\n")
        f.write("=" * 52 + "\n")
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Train Bagging Ensemble on Heart Failure dataset"
    )
